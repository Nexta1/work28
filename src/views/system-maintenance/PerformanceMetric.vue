  ...
  <div class="performance-metric-container">
    <div class="filter-control-bar">
      <el-form :inline="true" :model="queryForm" size="mini">
        <el-form-item label="指标名称">
          <el-input v-model="queryForm.metricName" placeholder="请输入指标名称" clearable style="width: 180px" />
        </el-form-item>
        <el-form-item label="数据类型">
          <el-select v-model="queryForm.dataType" placeholder="请选择" clearable style="width: 120px">
    ...
    <div class="table-content-wrapper" v-loading="loading">
      <el-table :data="tableData" style="width: 100%" height="100%">
        <el-table-column prop="performanceMetricId" label="指标ID" width="100" fixed align="center" show-overflow-tooltip />
        ...
        <el-table-column label="窗口配置" width="150" align="center">
          <template slot-scope="scope">{{ formatWindowConfig(scope.row.windowType, scope.row.windowLength) }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作时间" width="165" align="center">
          <template slot-scope="scope">{{ formatDateTime(scope.row.opTime) }}</template>
        </el-table-column>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="700px" :close-on-click-modal="false">
      <el-form ref="form" :model="formData" :rules="formRules" label-width="120px" size="small">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="指标名称" prop="metricName"><el-input v-model="formData.metricName" placeholder="请输入指标名称" /></el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="指标单位" prop="metricUnit"><el-input v-model="formData.metricUnit" placeholder="如：ms、%" /></el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="源数据表" prop="srcTableName"><el-input v-model="formData.srcTableName" placeholder="请输入表名" /></el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="源数据列" prop="srcColumnName"><el-input v-model="formData.srcColumnName" placeholder="请输入列名" /></el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="窗口长度" prop="windowLength"><el-input-number v-model="formData.windowLength" :min="1" :max="1000" placeholder="请输入窗口长度" style="width: 100%" /></el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-form-item label="分组列名" prop="groupColumnName"><el-input v-model="formData.groupColumnName" placeholder="可选" /></el-form-item>
        <el-form-item label="指标描述" prop="metricMemo"><el-input v-model="formData.metricMemo" type="textarea" :rows="3" placeholder="请输入指标描述" /></el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      dialogVisible: false,
      dialogTitle: '新增指标',
      formData: { performanceMetricId: null, dataType: '', metricName: '', metricUnit: '', srcTableName: '', srcColumnName: '', windowType: '', windowLength: 1, accumulateMethod: '', groupColumnName: '', metricMemo: '' },
      formRules: {
  ...
  methods: {
    async loadOptions() {
      try {
        const [dataTypeRes, windowRes, accumulateRes] = await Promise.all([dataTypes(), windowTypes(), accumulateMethods()])
        this.dataTypeOptions = dataTypeRes.data || []
    ...
    async loadData() {
      ...
      } catch (e) { console.error('加载性能指标失败:', e); this.$message.error('数据加载失败') } finally { this.loading = false }
    },
    handleSearch() { this.page.pageNum = 1; this.loadData() },
    resetQuery() { this.queryForm = { metricName: '', dataType: '' }; this.handleSearch() },
    ...
    openDialog(type, row = null) {
      ...
      if (type === 'edit' && row) {
        this.formData = { performanceMetricId: row.performanceMetricId, dataType: row.dataType, metricName: row.metricName, metricUnit: row.metricUnit || '', srcTableName: row.srcTableName, srcColumnName: row.srcColumnName, windowType: row.windowType, windowLength: row.windowLength || 1, accumulateMethod: row.accumulateMethod, groupColumnName: row.groupColumnName || '', metricMemo: row.metricMemo || '' }
      } else {
        this.formData = { performanceMetricId: null, dataType: '', metricName: '', metricUnit: '', srcTableName: '', srcColumnName: '', windowType: '', windowLength: 1, accumulateMethod: '', groupColumnName: '', metricMemo: '' }
      }
    ...
    handleSubmit() {
      this.$refs.form.validate(async valid => {
        ...
        try {
          ...
          if (this.formData.performanceMetricId) { await apiUpdate('performanceMetric', submitData); this.$message.success('修改成功') } else { await apiAdd('performanceMetric', submitData); this.$message.success('新增成功') }
    ...
    handleDelete(row) {
      this.$confirm(`确定要删除指标 [${row.metricName}] 吗？`, '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(async () => { try { await mainDelete('performanceMetric', row.performanceMetricId); this.$message.success('删除成功'); this.loadData() } catch (e) { console.error('删除失败:', e); this.$message.error('删除失败') } })
        .catch(() => {})
    },
    formatWindowConfig(windowType, windowLength) { if (!windowType || !windowLength) return '--'; return `${windowType}(${windowLength})` },
...
<style scoped>
.performance-metric-container { width: 100%; height: 100%; display: flex; flex-direction: column; padding: 16px; box-sizing: border-box; background-color: transparent; }
