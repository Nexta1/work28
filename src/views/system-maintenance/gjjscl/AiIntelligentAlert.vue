  ...
  <div class="ai-topology-container">
      ...
      <div class="ai-ticker-wrap">
        <div class="ai-ticker-track">
          ...
          <span class="ticker-text"
            >AI 推理引擎运行中 · 已检测 <b>4</b> 条告警链</span
          >
          <span class="ticker-divider">|</span>
          <span class="ticker-item pulse-red">●</span>
          <span class="ticker-text"
            >网络层告警 <b>2</b> · 平台层告警 <b>3</b> · 设备层告警
            <b>5</b></span
          >
          <span class="ticker-divider">|</span>
          <span class="ticker-item pulse-yellow">★</span>
          <span class="ticker-text"
            >置信度最高 <b>96.3%</b> · 模式库 v2.4.0</span
          >
        </div>
      </div>
    ...
    <div class="ai-metrics-dashboard">
      <div class="metric-card mc-network">
        <div class="mc-icon">
          <Icon icon="lucide:radio" :size="18" color="#3b82f6" />
        </div>
        <div class="mc-body">
      ...
      <div class="metric-card mc-platform">
        <div class="mc-icon">
          <Icon icon="lucide:cpu" :size="18" color="#a855f7" />
        </div>
        <div class="mc-body">
      ...
      <div class="metric-card mc-device">
        <div class="mc-icon">
          <Icon icon="lucide:radio-receiver" :size="18" color="#f97316" />
        </div>
        <div class="mc-body">
      ...
      <div class="metric-card mc-chain">
        <div class="mc-icon">
          <Icon icon="lucide:git-branch" :size="18" color="#10b981" />
        </div>
        <div class="mc-body">
      ...
      <div class="ai-analysis-sidebar">
        <div class="sidebar-section">
          <div class="sidebar-section-title">
            <Icon
              icon="lucide:search"
              :size="13"
              style="vertical-align: middle; margin-right: 4px; color: #a855f7"
            />
            AI 告警链诊断
          </div>
          <div v-if="selectedNode" class="diagnosis-panel">
            <div class="diag-header">
              <span class="diag-badge" :class="'badge-' + selectedNode.layer">{{
                layerLabel(selectedNode.layer)
              }}</span>
          ...
          <div v-else class="diag-placeholder">
            <Icon
              ...
              :size="24"
              style="color: #1e293b; margin-bottom: 8px"
            />
        ...
        <div class="sidebar-section">
          <div class="sidebar-section-title">
            <Icon
              ...
              :size="13"
              style="vertical-align: middle; margin-right: 4px; color: #f97316"
            />
            异常演进趋势
          </div>
        ...
        <div class="sidebar-section">
          ...
          <div class="event-stream">
            <div
              v-for="(evt, idx) in aiEvents"
              :key="idx"
              class="event-item"
              :class="'evt-' + evt.severity"
            >
              <span class="evt-dot"></span>
              <span class="evt-time font-num">{{ evt.time }}</span>
              <span class="evt-layer-tag" :class="'layer-' + evt.layer">{{
                evt.layer
              }}</span>
      ...
      aiEvents: [
        {
          ...
          severity: 'critical',
          layer: '网络',
          text: '数据链A链路中断 → 级联影响预警机平台'
        },
        {
          time: '09:26:42',
          severity: 'major',
          layer: '平台',
          text: '预警机平台脱联 → 下属传感器全部离线'
        },
        {
          time: '09:25:10',
          severity: 'major',
          layer: '设备',
          text: '机载雷达告警 → 通信中断导致目标丢失'
        },
        {
          time: '09:24:08',
          severity: 'minor',
        ...
        },
        {
          ...
          severity: 'critical',
          layer: '网络',
          text: '通信骨干网节点 #1024 心跳丢失'
        },
        {
          time: '09:15:00',
          severity: 'major',
          layer: '平台',
          text: '指挥车平台 GNSS 信号受到干扰'
        },
        {
          time: '09:08:22',
          severity: 'warning',
    ...
    initGraph() {
      this.graph = new Graph({
        ...
        grid: {
          ...
          type: 'doubleMesh',
          args: [
            {color: '#0d1a30', thickness: 1},
            {color: '#142240', thickness: 1, factor: 4}
          ]
        },
        connecting: {
          connector: 'smooth',
          ...
          attrs: {
            line: {
              ...
              style: 'animation: dot-flow 2s linear infinite'
            }
          }
        },
        interacting: false
    ...
    renderTopology() {
      ...
      const nodes = [
        // ======== 网络层 (左) ========
        {
          ...
          alertType: '🔴 链路中断告警',
          diagnosis:
            '数据链A于09:27:35完全中断。AI 分析: 物理层信号丢失, 疑似节点#1024被压制干扰。过去5分钟信噪比从12dB骤降至-3dB。',
          suggestion:
            '1. 立即切换至数据链B备用通道\n2. 启动跳频抗干扰模式\n3. 派出电子战单元侦察干扰源',
          affected: '2个平台脱联, 5个传感器离线'
        },
        {
          id: 'net-2',
          x: 50,
          ...
          alertType: '🟡 节点心跳丢失',
          diagnosis:
            '骨干网节点#1024心跳超时。AI 时序分析显示该节点CPU利用率在09:20-09:25期间从32%飙升至97%, 随后失联。',
          suggestion:
            '1. 远程重启节点#1024\n2. 检查供电和散热\n3. 冗余路径已自动切换',
          affected: '影响3个子网段路由'
        },
        ...
        // ======== 平台层 (中) - 隶属于网络 ========
        {
          ...
          alertType: '🔴 平台脱联告警',
          diagnosis:
            '预警机平台与数据链A断开连接。AI 关联分析: 数据链A中断(09:27:35) → 预警机脱联(09:27:38) → 机载传感器离线(09:27:42)。根因为数据链A链路故障。',
          suggestion:
            '1. 等待数据链A恢复\n2. 尝试通过通信骨干网迂回接入\n3. 启动机载备用数据链终端',
          affected: '3个传感器离线'
        },
        {
          ...
          alertType: '🟡 GNSS干扰告警',
          diagnosis:
            '指挥车GNSS信号受到电磁干扰, 定位精度从0.5m恶化至25m。AI 模式匹配为车载式欺骗干扰, 特征码 #JAM-2026-003。',
          suggestion:
            '1. 启用惯性导航+北斗双模备份\n2. 电子战分队定位干扰源方位\n3. 上报指挥中心调整部署',
          affected: '武器系统瞄准偏差'
        },
        {
          ...
          confidence: 8.2,
          alertType: '🟢 运行正常',
          diagnosis: '发射车平台所有指标正常, 通信链路冗余正常。',
          suggestion: '无需处理。',
          affected: ''
        },
        ...
        // ======== 设备/传感器/武器层 (右) ========
        {
          ...
          alertType: '🔴 传感器离线',
          diagnosis:
            '预警机机载雷达因平台脱联而离线。最后数据: 探测到3批不明目标, 距离120km。',
          suggestion: '1. 待平台恢复后自动重连\n2. 切换至另一预警机接力监视',
          affected: '目标跟踪中断'
        },
        {
          id: 'dev-2',
          ...
          alertType: '🟡 信号异常',
          diagnosis:
            '光电传感器 #03 红外信号出现周期性噪声, 疑似受到激光致眩。频率特征与已知激光武器 #LWS-2024 匹配度78%。',
          suggestion:
            '1. 关闭该传感器光学窗口\n2. 切换至雷达跟踪模式\n3. 记录干扰源方位'
        },
        {
          ...
          alertType: '🟡 通信超时',
          diagnosis:
            '武器系统 #03 通信自检超时, 导弹与发射车之间火控链路延迟从5ms增至120ms。AI 判断为GNSS干扰的间接影响。',
          suggestion:
            '1. 切换至有线火控模式\n2. 确认导弹自身惯导状态\n3. 上报待发射武器状态'
        },
        {
        ...
        },
        {
          ...
          confidence: 65.3,
          alertType: '🟡 链路质量下降',
          diagnosis: '发射车数据链终端信号强度下降40%, 误码率从1e-6升至1e-3。',
          suggestion: '1. 检查天线方位\n2. 增大发射功率\n3. 切换至定向天线模式'
        }
      ]
      ...
      nodes.forEach(item => {
        ...
        let bodyFill = '#0f1930',
          bodyStroke = '#1e3a5f',
          iconBgFill = 'rgba(56,189,248,0.08)',
          iconBgStroke = 'rgba(56,189,248,0.15)',
          labelColor = '#94a3b8',
          subColor = '#94a3b8',
          glow = ''
        if (isErr) {
          bodyFill = 'rgba(239,68,68,0.07)'
          bodyStroke = '#ef4444'
          iconBgFill = 'rgba(239,68,68,0.15)'
          iconBgStroke = 'rgba(239,68,68,0.3)'
          labelColor = '#fca5a5'
          subColor = '#ef4444'
          glow = 'filter:drop-shadow(0 0 16px rgba(239,68,68,0.6))'
        } else if (isWarn) {
          bodyFill = 'rgba(249,115,22,0.05)'
          bodyStroke = '#f97316'
          iconBgFill = 'rgba(249,115,22,0.12)'
          iconBgStroke = 'rgba(249,115,22,0.25)'
          labelColor = '#fdba74'
          subColor = '#f97316'
        ...
        series: [
          {
            ...
            areaStyle: {
              color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                {
                  offset: 0,
                  color: bad ? 'rgba(239,68,68,0.35)' : 'rgba(6,182,212,0.25)'
                },
...
.metric-card {
  background: linear-gradient(135deg, #0c1424 0%, #0a101e 100%);
...
.pulse-ring-delay {
  animation: pulse-ring-delay 1.5s ease-out 0.5s infinite;
}
</style>
